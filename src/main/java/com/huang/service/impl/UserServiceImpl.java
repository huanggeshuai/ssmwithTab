...
public class UserServiceImpl extends BaseService<UserServiceImpl> implements UserService {
    ...
    public void addUser(User user) {
        //我知道在这里加入自定义盐不太好，我也不清楚在哪里加入
        String userPassword=ShiroUtils.getPassword(user.getPassword(),sysInfoBean.getSelfSalt());
    ...
    @Override
    public void removeUserLock(List<String> userNameList) {
        if(!ObjectUtils.isEmpty(userNameList)){
            userNameList.forEach( userName -> {
                ehcache.evict(userName);
            });
        }
    }
}
